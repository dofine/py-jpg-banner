...
@click.option('--add-camera-logo', is_flag=True, show_default=True, default=True, help='Add camera logo or not.')
@click.option('--add-lens', is_flag=True, show_default=True, default=True, help='Add lens text or not.')
def add_border_to_image(image_path, output_path, add_camera_logo=True, add_lens=True):
    # 读取图片
    ...
    if camera_make is not None:
        ...
        if add_camera_logo:
            add_camera_logo = Image.open(f"logos/{camera_make.lower()}.jpg")
            # 富士的logo尺寸是 5000*961
            logo_aspect_ratio = add_camera_logo.width / add_camera_logo.height
            # 调整机型logo图片的大小
            logo_new_height = capture_date_box[3] + iso_text_size[3]
            logo_new_width = int(logo_new_height * logo_aspect_ratio)
            add_camera_logo = add_camera_logo.resize((logo_new_width, logo_new_height))
            # 如何决定 logo 的位置：y轴 - iso 和拍摄时间文本中间值，x轴 - 计算位置
            if iso_text_xy:
                logo_x = iso_text_xy[0] - logo_new_width - 10
            else:
                logo_x = int((image.width - add_camera_logo.width) / 2)
            new_image.paste(add_camera_logo, (logo_x, banner_start_y))  # 将 logo 图片粘贴在边框内
